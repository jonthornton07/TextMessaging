...
public class MessageActivity extends FragmentActivity{
	...
	public boolean onOptionsItemSelected(final MenuItem item) {
		...
		if ((item.getItemId() == android.R.id.home) && !mSlidingLayout.isOpen()) {
			...
			return true;
		}else if ((item.getItemId() == android.R.id.home) && mSlidingLayout.isOpen()) {
		...
		mSlidingLayout.setPanelSlideListener(new PanelSlideListener(){
			...
			public void onPanelOpened(final View panel) {
				if (mSlidingLayout.isSlideable()) {
					getSupportFragmentManager().findFragmentById(R.id.content_frame).setHasOptionsMenu(false);
					getSupportFragmentManager().findFragmentById(R.id.list_pane).setHasOptionsMenu(true);
				} else {
					getSupportFragmentManager().findFragmentById(R.id.content_frame).setHasOptionsMenu(true);
